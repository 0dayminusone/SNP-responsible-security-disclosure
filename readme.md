...
During my personal port scan using Shodan, I stumbled upon a server with IP the address `[redacted]` with an apache HTTP fileserver belonging to the spanish government, I performed a reverse DNS lookup

Using the `dig` tool, I did a reverse DNS lookup on the flagged IP address:

![dig command usage](./media/dig-example.png)

The reverse lookup pointed to a CNAME record, which then pointed to the domain: `pki.policia.es` "pki" in this context means, public key infrastructure
...
I visited the webserver and found that it contained documents that did not appear to contain sensitive information, and was probably purposely public facing, hence "**pki**.policia.es"
...
I then involved my spanish speaking friend who has experience with responsible vulnerability disclosure, I provided all of the information which I collected, and asked if it was meant to be public facing and if not, he could contact someone to remediate it.

One of the paths my friend tested was, `/.ssh` (the root-level SSH directory of the file server), it returned a successful HTTP 200 response code. This directory contained SSH keys used for server authentication and access

![](./media/ssh-screenshot.png)

## Responsible Disclosure

Upon discovering the exposed SSH keys, I immediately recognized the serious security implications. My native Spanish speaking friend then contacted the Spanish National Police with his findings, prompting them to remediate and fix this issue.
